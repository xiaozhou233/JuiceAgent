/* injector_client.c
   Console injector client in C.
   Usage:
     injector_client.exe <pid> <dll_path_to_inject> [injector_lib_path]
   If pid not provided, program runs "jps -l" and prompts for PID.
*/
...
/* Execute "jps -l" to help find JVM PIDs. */
static void ExecuteJps(void) {
    /* Use _popen to run jps -l and print output */
    FILE *pipe = _popen("jps -l", "r");
    if (!pipe) {
        printf("Error: failed to run jps -l (is JDK's bin on PATH?).\n");
        return;
    }
    printf("=== jps -l output ===\n");
    char buf[256];
...
int main(int argc, char **argv) {
    ...
    if (argc >= 3) {
        ...
        /* Default dll_path = currentdir\\libagent.dll */
    ...
    printf("[*] Calling inject(pid=%d, dllPath=\"%s\", NULL)\n", pid, dll_path);
    int ret = inject(pid, dll_path, NULL);
